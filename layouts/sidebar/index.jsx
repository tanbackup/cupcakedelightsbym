...
const Sidebar = ({ session, isAdmin, isCustomer, isUserPage, isSidebarOpen, onSidebarClose }) => {
	...
	return (
		<>
			...
			<chakra.aside bg="system" position="fixed" top={{ base: 0, lg: isAdmin ? 'auto' : isUserPage ? 'auto' : 0 }} left={{ base: isSidebarOpen ? 0 : -256, lg: isAdmin ? 'auto' : isSidebarOpen ? 0 : isUserPage ? 'auto' : -256 }} h={{ base: 'full', lg: isAdmin ? 'calc(100% - 72px)' : isUserPage ? 'calc(100% - 72px)' : 'full' }} w={256} transition="0.4s ease-in-out left" zIndex={{ base: 100, lg: 99 }}>
						...
						{isAdmin ? (
							...
							<Flex direction="column" gap={1}>
								...
								<NextLink href="/user/ratings" passHref>
									<Link as="span" display="block" py={2} lineHeight={6} active={router.pathname.includes('ratings') ? 1 : 0} onClick={onSidebarClose}>
										<Flex align="center" gap={3}>
											<Icon as={FiStar} boxSize={4} />
							...
							<Flex direction="column" gap={1}>
								<NextLink href="/" passHref>
									<Link as="span" display="block" py={2} lineHeight={6} active={router.pathname === '/' ? 1 : 0} onClick={onSidebarClose}>
										Home
									</Link>
								</NextLink>

								<NextLink href="/shop" passHref>
									<Link as="span" display="block" py={2} lineHeight={6} active={router.pathname.includes('shop') ? 1 : 0} onClick={onSidebarClose}>
										Shop
									</Link>
								</NextLink>
