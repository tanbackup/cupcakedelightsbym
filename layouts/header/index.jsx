...
const Header = ({ session, isAdmin, isCustomer, onSidebarOpen }) => {
	...
	return (
		<chakra.header bg="system" position="sticky" top={0} outline="1px solid" outlineColor="border" shadow={isScrolling && 'sm'} transition=".4s" zIndex={{ base: 99, lg: 100 }} _dark={{ shadow: isScrolling && 'dark-xl' }}>
			<Flex align="center" gap={6} mx="auto" px={6} h="72px" w="full" maxW={isAdmin ? 1536 : 1280}>
				<Flex flex={{ base: 1, lg: 'none' }} justify="start" align="center">
					<NextLink href="/" passHref>
						<Image display={{ base: 'none', lg: 'block' }} alt="logo" src="/assets/logo-brand.svg" />
					</NextLink>

					<IconButton display={{ base: 'flex', lg: 'none' }} variant="outline" icon={<FiMenu size={20} />} onClick={onSidebarOpen} />
				</Flex>
						...
						{isAdmin && (
							<>
								<NextLink href="/admin/dashboard" passHref>
									<Link as="span" active={router.pathname === '/admin/dashboard' ? 1 : 0}>
										Dashboard
									</Link>
								</NextLink>

								<NextLink href="/admin/products" passHref>
									<Link as="span" active={router.pathname === '/admin/products' ? 1 : 0}>
										Products
									</Link>
								</NextLink>

								<NextLink href="/admin/accounts" passHref>
									<Link as="span" active={router.pathname === '/admin/accounts' ? 1 : 0}>
										Accounts
									</Link>
								</NextLink>
							</>
						...
						{isCustomer && (
							<>
								...
								<NextLink href="/shop" passHref>
									<Link as="span" active={router.pathname.includes('shop') ? 1 : 0}>
										Shop
									</Link>
								</NextLink>

								<NextLink href="/reviews" passHref>
									<Link as="span" active={router.pathname === '/reviews' ? 1 : 0}>
										Reviews
									</Link>
								</NextLink>

								<Link href="/#contact" active={router.pathname === '/contact' ? 1 : 0}>
									Call Us
								</Link>

								<NextLink href="/cart" passHref>
									<Link as="span" active={router.pathname === '/cart' ? 1 : 0}>
					...
					{session ? (
						<Menu>
							...
							<MenuList w={256}>
								...
								<a target="_blank" rel="noreferrer" href="https://facebook.com/cupcakedelightsby.m">
									<MenuItem icon={<FiFacebook size={16} />}>Facebook</MenuItem>
								</a>

								<a target="_blank" rel="noreferrer" href="https://instagram.com/cupcakedelightsby_m">
						...
						<>
							<Button display={{ base: 'none', lg: 'flex' }} variant="outline" leftIcon={<Google size={20} />} onClick={() => signIn('google')}>
								Sign in
							</Button>
